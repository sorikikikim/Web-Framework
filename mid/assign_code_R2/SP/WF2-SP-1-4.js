...
(async () => {
    try {
        ...
        await fsPromises.access(`.${path.sep + folderName}`, constants.F_OK | constants.W_OK | constants.R_OK);
        ...
        if (err.code==='ENOENT') {
            ...
            await fsPromises.mkdir(`.${path.sep + folderName}`);
    ...
    try {
        ...
        const fileHandle = await fsPromises.open(`.${path.sep + folderName + path.sep + filename1}`, 'w');
        console.log(`${folderName + path.sep + filename1} file (${fileHandle.fd}) is available.\n`);

        // 3. 생성한 파일명 work1.txt을 homework1.txt로 변경하고, 결과를 출력합니다.
        await fsPromises.rename(`.${path.sep + folderName + path.sep + filename1}`, `.${path.sep + folderName + path.sep + filename2}`);
        console.log(`${filename1} was changed to ${filename2} at .${path.sep + folderName}\n`);

        // 4. homework1.txt 파일에 다음의 buffer의 내용을 기록하고 결과를 출력합니다.
        await fsPromises.writeFile(`.${path.sep + folderName + path.sep + filename2}`, buffer);
        console.log(`${filename2} written.\n`);

        // 5. homework1.txt 파일의 내용을 문자열로 화면에 출력합니다.
        const data = await fsPromises.readFile(`.${path.sep + folderName + path.sep + filename2}`);
